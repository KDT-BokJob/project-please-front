...
export const jobPostingFormSchema2 = z.object({
  prefered_nationality: z.string({ required_error: '희망하는 국적을 선택해주세요.' }).refine((val) => val !== '', {
    message: '희망하는 국적을 선택해주세요.',
  }),
  count: countSchema(),
  salary_type: z.string({ required_error: '필수선택' }).refine((val) => val !== '', {
    message: '필수선택',
  }), //수정필요
  salary: SalarySchema(),
  work_type: z.string({ required_error: '근무 형태를 선택해주세요.' }).refine((val) => val !== '', {
  ...
  work_start_hour: z.string({ required_error: '근로 시작 시간을 선택해주세요.' }).refine((val) => val !== '', {
    message: '필수선택',
  }),
  work_end_hour: z.string({ required_error: '근로 마감 시간을 선택해주세요.' }).refine((val) => val !== '', {
    message: '필수선택',
  }),
  is_worktime_flexible: z.coerce.boolean(),
  is_workperiod_flexible: z.coerce.boolean(),
  work_location: z.string({ required_error: '근무 장소를 입력해주세요.' }).refine((val) => val !== '', {
    message: '근무 장소를 입력해주세요.',
  }),
})
...
export const jobPostingFormPreview = z
  .object({
  ...
  .and(
    z.object({
      job: z.string(),
      visa_type: z.string(),
      isVisa_transform: z.string({ required_error: '비자전환 가능 여부를 체크해주세요.' }),
  ...
  .and(
    z.object({
      ...
      work_days_week: availableWorkDaysSchema(),
