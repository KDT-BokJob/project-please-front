...
export default function page({ params: { locale } }: { params: { locale: string } }) {
  ...
  useEffect(() => {
    const translate = async () => {
      const { t, language } = await initTranslations(locale, ['resumeCert', 'common'])
      tl.current = t
  ...
  return (
    <>
      <h1 className={'title-m mx-auto mt-[135px] mb-[70px]'}>🗃️{tl.current('Certifications')}</h1>
      <Form {...form}>
          ...
          {fieldArray.fields.map((field, index: number) => (
            ...
            >
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{tl.current && tl.current('Certificate Name')} *</FormLabel>
                    <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{tl.current && tl.current('Certification Authority')} *</FormLabel>
                    <FormControl>
                      <Input type="text" {...field} />
                    </FormControl>
                    <FormMessage />
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-col">
                    <FormLabel>{tl.current && tl.current('Acquisition Date')}</FormLabel>
                    <Popover>
